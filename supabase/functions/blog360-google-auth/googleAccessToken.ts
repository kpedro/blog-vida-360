 ...
 * Mantenha alinhado com `google-calendar-create-event/googleAccessToken.ts` e `google-workspace-api/googleAccessToken.ts`.
 */
...
export async function getValidGoogleAccessToken(
  ...
  clientId: string,
  clientSecret: string
): Promise<{ access_token: string } | { error: string; status: number }> {
  ...
  if (tokenError || !tokenData) {
    return { error: 'Google não conectado. Conecte sua conta Google nas Integrações.', status: 400 };
  }
  ...
  if (now >= expiresAt) {
    if (!tokenData.refresh_token) {
      return {
        error: 'Sessão Google expirada. Reconecte sua conta nas Integrações.',
        status: 400,
      };
    }
    ...
    try {
      ...
      return {
        error: 'Não foi possível renovar o acesso Google. Reconecte nas Integrações.',
        status: 500,
      };
    }
  }
